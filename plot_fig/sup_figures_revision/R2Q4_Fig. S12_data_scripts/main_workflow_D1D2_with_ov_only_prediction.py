...
if __name__ == '__main__':
    ...
    sc_dataset_ids = ['Geistlinger2020_Ovarian_3CA']
    ...
    #                               'PRAD', 'THCA', 'UCEC']
    cancer_types_for_filtering = [i for i in cancer_types if i != 'OV']
    ...
    cell_prop_prior = {'B Cells': (0, 1), 'CD4 T': (0, 1), 'CD8 T': (0, 1), 'T Cells': (0, 1),
    ...
    cell_subtype_col = 'cell_type'  # the column name of cell subtype in the merged sc dataset
    single_cancer_type = 'ov'  # ovarian cancer in ascites, used by Scaden
    simu_ds_dir = f'simulated_bulk_cell_from_single_scRNAseq_dataset'
    sc_ds = sc_dataset_ids[0]
    ...
    dataset2parameters = {
        ...
        # f'SCT_POS_N100_test_{sc_ds}': {'n_each_cell_type': 100, 'cell_type2subtype': cell_type2subtypes,
        #                                'test_set': True},
        f'SCT_POS_N3K_{sc_ds}': {'n_each_cell_type': 3000, 'cell_type2subtype': cell_type2subtypes},
        ...
        f'Mixed_N30K_segment_{sc_ds}': {'sc_dataset_ids': sc_dataset_ids,
                                         'cell_type2subtype': cell_type2subtypes,
                                         'n_samples': 30000,
                                         'sampling_method': 'segment',
                                         'filtering': True,
    ...
    merged_sc_dataset_file_path = os.path.join(deside_data_dir, 'single_cell',
                                               f'Geistlinger2020_Ovarian_3CA_log2cpm1p.h5ad')
    ref_gene_list_file_path = os.path.join(deside_data_dir, 'single_cell', 'merged_12_sc_datasets_gene_list.csv')
    ...
    result_root_dir = os.path.join('results', f'whole_workflow_20231110_12ds_subtypes')
    ...
    sct_dataset_file_path = os.path.join(sc_dataset_dir,
                                         f'simu_bulk_exp_SCT_POS_N3K_{sc_ds}_log2cpm1p.h5ad')
    if debug:
    ...
    current_project_id = f'DeSide_03_new_ds_subtypes_all_range'
    ...
    #         # TODO: add 2 parameters for the SCT dataset generation, minimum_n_base and ref_gene_list
    ...
    #                                    minimum_n_base=30, ref_gene_list_file_path=ref_gene_list_file_path)
    #     else:
    ...
    #         d2_dir = os.path.join(deside_data_dir, 'simulated_bulk_cell_dataset_subtypes_all_range',
    #                               'segment_12ds_no_filtering_n_base100_median_gep', 'D2')
    #         # check_dir(d2_dir)
    ...
    #         if os.path.exists(high_corr_gene_file_path):
    #             print(f'High correlation gene list file existed in: {high_corr_gene_file_path}')
    ...
    training_set_list = [
        ...
        # f'Mixed_N30K_segment_filtered_{sc_ds}',  # filtered by both cell-level and gene-level
        # 'Mixed_N100K_segment_filtered-SCT_POS_N100',  # filtered by both cell-level and gene-level

        f'Mixed_N100K_segment_without_filtering',  # not filtered by both GEP-level and gene-level
    ...
    # evaluation_dataset2path[sct_test_set] = os.read_count_file_path.join(deside_data_dir, '',
    ...
    for inx, training_set_name in enumerate(training_set_list):
        ...
        run_step4(tcga_data_dir=tcga_data_dir, cancer_types=cancer_types, log_file_path=log_file_path,
                  ...
                  sample2cancer_type_file_path=tcga2cancer_type_file_path, bulk_tpm_file_path=tcga_merged_tpm_file_path)
